...
void Util::DrawBox(int X, int Y, int W, int H, const ImColor color, int thickness) {
	ImGui::GetOverlayDrawList()->AddRect(ImVec2(X, Y), ImVec2(X + W, Y + H), color, 0.0f, 0, thickness);
	ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(X, Y), ImVec2(X + W, Y + H), ImColor(0, 0, 0, 150));
}
void Util::DrawLine(ImVec2 p1, ImVec2 p2, const ImColor color, int thickness) {
	ImGui::GetOverlayDrawList()->AddLine(p1, p2, color, thickness);
}
...
void Util::KillProc(std::string name)
{
	std::string command = _s("taskkill /F /T /IM ") + name + _s(" >nul 2>nul");
